...
import { scrollPosterIntoView, tryExpandHubRowFromPoster } from '../utils/hub-row-nav';
...
let focusCacheGeneration = 0;



export function invalidateFocusCache(): void {
...
export function initFocusRoot(root: HTMLElement): () => void {
  ...
  const onKeyDown = (event: KeyboardEvent) => {
    ...
    if (root.querySelector('.player-screen')) {
      return;
    }
...
function pickHorizontalRowSibling(
  ...
  const items = Array.from(
    ...
    row.querySelectorAll<HTMLElement>('.poster-card.focusable, .focusable[tabindex="0"]'),
...
function visibleFocusables(root: HTMLElement): HTMLElement[] {
  ...
  if (
    ...
    focusCache.root === root &&

    focusCache.generation === focusCacheGeneration
  ...
  focusCache = { root, list, generation: focusCacheGeneration };
...
export function captureFocus(root: HTMLElement): string | null {
...
export function restoreFocus(root: HTMLElement, token: string | null): void {
  ...
  if (type === 'id') {
    ...
    el = root.querySelector<HTMLElement>(`#${cssEscape(value)}`);

  } else {

    el = root.querySelector<HTMLElement>(`[${type}="${cssEscapeAttr(value)}"]`);
